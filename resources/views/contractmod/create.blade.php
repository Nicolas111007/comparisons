...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{__("Ajout de contrat")}}</title>
    ...
    <body class="antialiased">
        ...
        <div class="flex items-center justify-center mb-36 mt-36">
            <div class="bg-sky-400 p-5 rounded-lg w-fit">
                <div class="pb-5 max-w-7xl mx-auto sm:px-6 lg:px-8 font-bold text-center underline"><h2>{{__("Données du contrat à ajouter")}}</h2></div>
                <form method="post" action="{{route('contractmod.store')}}">
                    @csrf
                    
                    <table class="m-auto mb-10">
                        <tr>
                            <th class="px-3 text-white" colspan=8>{{__("Données du contrat")}}</th>
                        </tr>
                        <tr>
                            <th class="px-3 text-white"><label for="suppliers_id">{{__("ID du Fournisseur")}}</label></th>
                            <th class="px-3 text-white"><label for="energy_type_id">{{__("ID type d'énergie")}}</label></th>
